...
1. 在boot_alloc()函数处迷惑了很长时间：为什么其中会出现虚拟地址？
   ...
   􏵽􏼶􏶕􏶚􏼟􏶩􏹈􏺲􏹦􏸁􏻹􏵹􏵾􏵼􏶕􏶚􏵫􏹎􏻋􏶘􏷋􏷋􏵵􏵩􏶘􏷋􏷋􏵵􏵼􏶰􏶡􏶵􏷈􏶺􏵽􏺥􏶻􏹠􏸉􏵾􏵶 而且，一旦我们处于保护模式（也就是我们在进入boot/boot.S中做的第一件事），就没有办法直接使用线性或物理地址。所有存储器引用被理解为虚拟地址并由MMU转换，这意味着C中的所有指针都是虚拟地址。
...
#### Exercise 2：分段（segmentation）和页转换（page translation）。
