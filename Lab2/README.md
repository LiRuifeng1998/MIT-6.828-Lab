...
* [JOS实验lab2 内存管理](#jos实验lab2-内存管理)
      * [零、实验要求](#零实验要求)
      * [一、实验简介](#一实验简介)
         * [(一)物理页面管理](#一物理页面管理)
         * [(二)虚拟内存](#二虚拟内存)
      * [二、实验过程](#二实验过程)
         * [PART 0：预备知识](#part-0预备知识)
         * [PART 1：编写物理页分配器](#part-1编写物理页分配器)
            * [(1) &lt;inc/mmu.h&gt;](#1-incmmuh)
            * [(2) &lt;kern/pmap.h&gt;](#2-kernpmaph)
            * [(3）练习解答](#3练习解答)
         * [PART 2：虚拟内存](#part-2虚拟内存)
            * [(1)虚拟地址、线性地址、物理地址](#1虚拟地址线性地址物理地址)
            * [(2)内存管理方案](#2内存管理方案)
            * [(3)页表管理](#3页表管理)
         * [PART 3 内核地址空间](#part-3-内核地址空间)
      * [三、问题讨论](#三问题讨论)
      * [四、额外问题](#四额外问题)
...
3. page_lookup（）
   ...
   //
   struct Page *
   page_lookup(pde_t *pgdir, void *va, pte_t **pte_gestore)
   {
           // Fill this function in
...
1. 关于struct Page占字节的大小

   使用sizeof()输出结果为8，结构体如下
   ...
   有一个指针和一个2byte的uint16_t型变量，但是不清楚为什么这个指针占6byte。